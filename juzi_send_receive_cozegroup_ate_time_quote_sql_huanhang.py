...
async def receive_data(
    ...
    if "方工" in original_query:
        ...
        # 构建 mention 和 payload 文本
        mention_list = [f"@{data.contactName or ''}"] if data.contactName else []
        response_text = f"{' '.join(mention_list)}\n{bot_response}"
        data.payload.text = response_text
        data.payload.quoteMessageId = data.messageId
        data.payload.mention = [data.imContactId] if data.imContactId else []
        ...
        await insert_message_to_db(
            ...
            msg_type=0  # 消息类型设为0，表示包含“方工”
        )

    else:
        # 不包含“方工”时，直接插入数据库并不调用机器人
        await insert_message_to_db(
            contact_name=data.contactName or "",
            query_text=original_query,  # 插入客户的原始问题
            bot_response="",  # 没有机器人回复时留空
            room_topic=data.roomTopic or "",
            msg_type=1  # 消息类型设为1，表示不包含“方工”
        )
        log_to_file("Message does not contain the trigger word '方工'. Recorded with msg_type=1.")
