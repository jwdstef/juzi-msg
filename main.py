...
import time
import aiomysql
from get_product_link_te import *
...
# 记录日志到文件
def log_to_file(message: dict, level: str = "INFO", filename_prefix="log"):
    """
    按标准格式记录日志。
    """
...
async def process_message(data: MessageData, query: str, is_group: bool = False) -> dict:
    ...
    try:
        # 在修改 payload 之前保存原始文本
        original_text = data.payload.text  # 保存原始输入
        bot_response = call_bot(query)
        log_data["机器人响应"] = bot_response
        ...
        if any(response in bot_response for response in unable_responses) or \
                ("💾 资料链接:" in bot_response and "http" not in bot_response) or \
                "https://www" in bot_response or \
                "https://example" in bot_response:
            # 从原始查询中提取型号并传递完整的查询
            model_number = extract_model_number(query)
            if model_number != "未找到型号":
                bot_response = get_product_link_response(model_number, query)
            else:
                bot_response = "抱歉，我无法找到匹配的产品型号。请确保型号准确无误。"
        ...
        if is_group:
            ...
            data.payload.quoteMessageId = data.messageId  # 添加消息引用
        else:
        ...
        await insert_message_to_db(
            ...
            query_text=original_text,  # 使用保存的原始输入
